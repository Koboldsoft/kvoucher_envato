...
function kv_getCurrency(){
    ...
    if( empty( sanitize_text_field( get_option('kvoucher_plugin_company_textfiels') ) ) ){ $output = "€"; }
    
    else{
        
        if( strcmp( sanitize_text_field( get_option('kvoucher_plugin_company_textfiels')['currency'] ), 'euro' ) == 0 ){ $output = " €"; };
        
        if( strcmp( sanitize_text_field( get_option('kvoucher_plugin_company_textfiels')['currency'] ), 'dollar' ) == 0 ){ $output = " $"; };
        
        if( strcmp( sanitize_text_field( get_option('kvoucher_plugin_company_textfiels')['currency'] ), 'british_pound' ) == 0 ){ $output = " £"; };
